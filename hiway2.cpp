// https://oj.vnoi.llInfo/problem/hiway2
...
struct minCostFlow {
    ...
    bool in_queue[max_n];
    long long d[max_n]; int par[max_n];

    bool findAugmentPath(int s, int t) {
        fill(in_queue, in_queue + max_n, false);
        fill(d, d + max_n, llInf); fill(par, par + max_n, -1);
    ...
    pair<int, long long> maxFlow_minCost(int s, int t) {
...
void Process() {
    pair<int, long long> result = mcf.maxFlow_minCost(0, f);
